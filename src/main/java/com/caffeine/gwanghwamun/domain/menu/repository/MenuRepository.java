...
public interface MenuRepository extends JpaRepository<Menu, UUID> {
	...
	@Query("SELECT m FROM Menu m WHERE m.store.storeId = :storeId AND m.deletedAt IS NULL")
	Page<Menu> findByStoreIdAndNotDeleted(@Param("storeId") UUID storeId, Pageable pageable);

	@Query("SELECT m FROM Menu m WHERE m.store.storeId = :storeId AND m.deletedAt IS NULL AND m.isHidden = false")
	Page<Menu> findByStoreIdAndNotDeletedAndNotHidden(@Param("storeId") UUID storeId, Pageable pageable);
