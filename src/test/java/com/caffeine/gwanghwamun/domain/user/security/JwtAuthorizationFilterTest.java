...
class JwtAuthorizationFilterTest {
	...
	void authenticatedRequestWithJwt() throws Exception {
		...
		mockMvc
				.perform(get("/v1/test/protected").header(JwtUtil.AUTHORIZATION_HEADER, token))
				.andDo(print())
				.andExpect(status().isOk());
	}
	...
	void accessWithInvalidToken() throws Exception {
		...
		// When & Then
		mockMvc
				.perform(get("/v1/test/protected").header(JwtUtil.AUTHORIZATION_HEADER, invalidToken))
				.andDo(print())
	...
	void accessPublicEndpoint() throws Exception {
		...
		mockMvc.perform(get("/v1/test/public")).andDo(print()).andExpect(status().isOk()); // 200: 접근 허용
	}
	...
	void checkCustomerAuthoritySuccess() throws Exception {
		...
		mockMvc
				.perform(get("/v1/test/customer").header(JwtUtil.AUTHORIZATION_HEADER, token))
				.andDo(print())
				.andExpect(status().isOk());
	}
	...
	void checkCustomerAuthorityFail() throws Exception {
		...
		mockMvc
				.perform(get("/v1/test/manager").header(JwtUtil.AUTHORIZATION_HEADER, token))
				.andDo(print())
